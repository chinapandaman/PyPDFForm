...
def test_on_focused(static_pdfs, pdf_samples, js_samples, request):
    expected_path = os.path.join(pdf_samples, "docs", "test_on_focused.pdf")

    form = PdfWrapper(os.path.join(static_pdfs, "sample_template.pdf"))
    form.widgets["test"].on_focused_javascript = os.path.join(
        js_samples, "doc_examples", "test_on_focused.js"
    )
...
def test_off_focused(static_pdfs, pdf_samples, js_samples, request):
