...
def test_sample_font_fixtures_setup(font_directory, tinos_regular, tinos_bold, tinos_italic, tinos_bold_italic):
    assert os.path.isdir(font_directory)

    fonts = [
        value
        for key, value in locals().items() if isinstance(value, str) and key.startswith("tinos")
    ]
    for each in fonts:
