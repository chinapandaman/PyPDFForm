...
def test_draw_text_on_one_page_different_font(template_stream, pdf_samples, font_samples):
    with open(os.path.join(font_samples, "LiberationSerif-BoldItalic.ttf"), "rb+") as _f:
        PyPDFForm.register_font("LiberationSerif-BoldItalic", _f.read())

    with open(os.path.join(pdf_samples, "sample_pdf_with_drawn_text_different_font.pdf"), "rb+") as f:
        obj = PyPDFForm(template_stream).draw_text(
            "drawn_text", 1, 300, 225, "LiberationSerif-BoldItalic", 20, (1, 0, 0), 50, 50, 4
        )
